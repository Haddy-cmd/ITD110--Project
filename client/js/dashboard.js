...
function makeInitialAvatar(name, cls = 'product-img-fallback') {
...
/* ── View navigation ── */
const viewTitles = {
    homeView: 'Dashboard', inventoryView: 'Products',
    cartView: 'Cart', orderHistoryView: 'My Orders', profileView: 'Profile'
};
...
function renderInventory() {
    ...
    filtered.forEach(p => {
        ...
        const hasImg = p.image && (p.image.startsWith('img/') || p.image.startsWith('http') || p.image.startsWith('data:image/') || p.image.includes('.'));
        const imgHTML = hasImg
            ? `<div class="product-img-wrap"><img class="product-img" src="${p.image}" alt="${p.name}"
                  onerror="this.parentElement.innerHTML='${makeInitialAvatar(p.name).replace(/'/g, "\\'")}'"
               ></div>`
            : makeInitialAvatar(p.name);
        ...
        card.innerHTML = `${imgHTML}
            ...
            <div class="product-qty ${p.quantity <= 5 && p.quantity > 0 ? 'low-stock' : ''}">
                Stock: ${p.quantity}${p.quantity <= 5 && p.quantity > 0 ? ' — low' : ''}
            </div>
...
function addToCart(id, qty) {
    ...
    if (existing) {
        existing.qty = Math.min(existing.qty + qty, prod.quantity);
...
function renderCart() {
    ...
    cart.forEach((item, idx) => {
        ...
        const hasImg = item.product.image && (item.product.image.startsWith('img/') || item.product.image.startsWith('http') || item.product.image.startsWith('data:image/') || item.product.image.includes('.'));
        const thumb  = hasImg
            ? `<img class="cart-thumb" src="${item.product.image}" onerror="this.outerHTML='${makeInitialAvatar(item.product.name,'cart-initial').replace(/'/g, "\\'")}'">` 
            : makeInitialAvatar(item.product.name, 'cart-initial');
        ...
        tr.innerHTML = `<td>${thumb}</td>
            ...
            <td><div class="cart-qty-ctrl">
                ...
                <span>${item.qty}</span>
                <button class="cart-qty-inc" data-idx="${idx}" ${item.qty >= stock ? 'disabled style="opacity:0.3"' : ''}>+</button>
            </div></td>
    ...
    tbody.querySelectorAll('.remove-btn').forEach(btn => btn.addEventListener('click', () => {
        ...
        updateCartBadge(); renderCart(); showToast('Item removed');
    }));
}
...
document.getElementById('clearCartBtn').addEventListener('click', () => {
    ...
    cart = []; updateCartBadge(); renderCart(); showToast('Cart cleared');
});
...
function buildReceipt(bodyId, totalId) {
    ...
    cart.forEach(item => {
        ...
        const tr = document.createElement('tr');
        tr.innerHTML = `<td>${item.product.name}</td><td>${item.qty}</td><td>₱${item.product.price.toFixed(2)}</td><td>₱${sub.toFixed(2)}</td>`;
        tbody.appendChild(tr);
    });
    totalEl.textContent = `₱${grand.toFixed(2)}`;
    return grand;
...
document.getElementById('newPassword').addEventListener('input', function () {
    const s = [{ w:'0%',bg:'#303030' },{ w:'25%',bg:'#ef4444' },{ w:'50%',bg:'#f59e0b' },{ w:'75%',bg:'#3b82f6' },{ w:'100%',bg:'#22c55e' }];
